	...
	$(".submit-comment").on("click", function(){
		...
		if(content.length == 0){
			...
			$.post("",{
				...
				context.parents(".comment-section")
				.find(".comment-sec > ul")
				.append(
					`<li class="comment">
					<h3>${obj.name}</h3>
					<p>&nbsp; ${obj.conteudo}</p>
					<span><img src="images/clock.png" alt="">${obj.date}</span>
					</li>`);
					context.html("Comentar");
					context.removeAttr("disabled");
					context.parents(".comment-section").find(".comment-sec").removeAttr("hidden");
					let num_comments = context.parents(".post-bar").find(".comment-num").text();
					let new_num_comments = parseInt(num_comments) + 1;;
					context.parents(".post-bar").find(".comment-num").text(new_num_comments);
				});
			}
		...
		$(".btn-like").on("click", function(){
			...
			if (postBar.find(".btn-like i").attr("data-user_liked") == 0) {
				...
				let total_likes = postBar.find(".btn-like .likes_num").text();
				let new_total_likes = parseInt(total_likes) + 1;
				postBar.find(".btn-like .likes_num").text(new_total_likes);
			}
